...
# =========================================================
# AREA LOGIN & UPLOAD EXCEL DI HALAMAN UTAMA
# =========================================================
...
with col_log1:
    if not st.session_state['admin_logged_in']:
        with st.form("main_login_form"):
            ...
            if submitted:
                if u_name == "admin" and p_word == "admin123":
        ...
        if st.button("Logout (Keluar Admin)"):
            st.session_state['admin_logged_in'] = False
            st.session_state['uploaded_excel_file'] = None
            st.rerun()
...
with col_log2:
    if st.session_state['admin_logged_in']:
        st.markdown("### 📁 Perbarui Data Logsheet")
        uploaded_file = st.file_uploader("Unggah file Excel (.xlsx / .xls) yang baru:", type=["xlsx", "xls"])
        
        if uploaded_file is not None:
            st.session_state['uploaded_excel_file'] = uploaded_file
            st.success("🎉 File Excel berhasil diperbarui! Data di seluruh menu otomatis berubah.")
    else:
        st.info("""
        ℹ️ **Status: Guest (Tamu)**
        - Anda dapat melihat seluruh laporan, grafik, dan tabel secara bebas.
        - Untuk mengunggah file Excel baru, silakan lakukan **Login Admin** di sebelah kiri.
        """)